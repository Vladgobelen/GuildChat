...
print ("после переменных, но до функций")
function all_trim(s)
	return s:match( "^%s*(.-)%s*$" )
end
...
print ("после функций, но до получения первого символа")
...
print ("после получения первого символа, но до получения команды")
...
--проверка на существование всех таблиц
	...
	print ("координаты успешно назначены как ноль в таблице игрока в нике в квесте")
	if TDGq==nil then --поверка, что существует таблица с квестами
		TDGq={}
	end
	print ("после определения таблиц, кроме существования квеста")
