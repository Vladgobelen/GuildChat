...
SendChatMessage("*" .. nik .. ", простой или сложный?", "guild", nil, 1)
	...
	elseif message == "ВОЖДЬ, сдать" and nachalo~="*" then
		if TDG[sender][qAchiv]==nil or TDG[sender][qAchiv]=="9999" then
			SendChatMessage("*" .. sender .. ", у тебя нет взятых квестов.", "guild", nil, 1);
		else
			print ("111111")
			proverkaVypolneniya=TDG[sender][qAchiv]
			SendChatMessage(sender .. ", а сделал ли ты " .. proverkaVypolneniya .. " " .. GetAchievementLink(proverkaVypolneniya) .. "?", "guild", nil, 1)
		end
	elseif string.find (message, sender) and nachalo~="*" then
		proverkaVypolneniya=mysplit(message)
		if string.find(message, hsh) and string.find(message, "ага:") then
	...
	elseif str == "ВОЖДЬ, сдать доп квест!!!" and nachalo~="*" then
		if TDG[sender]["доп_квест"]~=nil and TDG[sender]["доп_квест"]~="9999" then
			...
			if rezDopQuest>kol then
				...
				SendChatMessage("*" .. sender .. " получает кусок карты (за куском обращаться отдельно.", "guild", nil, 1);
	...
	elseif string.find(message, "ВОЖДЬ") and string.find (message, "можно сделать:") and string.find(message, hsh)  then

		if TDG[sender][qAchiv]==nil or TDG[sender][qAchiv]=="9999" then
			vydat_q=mysplit(message)
			SendChatMessage(sender .. ", получи ачивку " .. vydat_q[5] .. " " .. GetAchievementLink(vydat_q[5]), "guild", nil, 1)
			prostye_q=vydat_q[5]
			TDG[sender][endQuests][prostye_q]="0"
			TDG[sender][qAchiv]=prostye_q
		else
			SendChatMessage("*" .. sender .. ", у тебя уже взят квест  " .. vydat_q[5] .. " " .. GetAchievementLink(vydat_q[5]), "guild", nil, 1)
		end
	...
	elseif message == "ВОЖДЬ, простой" and nachalo~="*" then
		if TDG[sender][endQuests][1]~=nil then
			countQ=tablelength(pQuests[1])
			for testQ=1, countQ do
				local x = math.random(1, countQ)
				ach=pQuests[1][x]
				if TDG[sender][endQuests][x]~="1" then
					SendChatMessage(sender .. ", покажи мне ачивку " .. ach .. " " .. GetAchievementLink(ach), "guild", nil, 1)
					break
				end
			end
		else
